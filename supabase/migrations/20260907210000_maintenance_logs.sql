-- maintenance_logs (+ tasks junction) — log-entry-only capture of ad hoc maintenance
-- events (filter change, sump flush, other), following the chemical_additions pattern
-- (data_source + event/entered timestamps, recorded_by defaulted server-side,
-- contributor/admin RLS). task_type is multi-select (a visit might include both a
-- filter swap and a sump flush) so it lives in its own junction table rather than an
-- array column, mirroring health_observations/health_observation_issues. Scoped down
-- from the roadmap's full "Maintenance scheduling" area (platform-architecture.md §4,
-- §6) — no recurrence/due-date maintenance_tasks table, no interval config, no
-- due/overdue computation, just the event log.
...
create table core.maintenance_log_tasks (
  maintenance_log_id int not null references core.maintenance_logs (id) on delete cascade,
  task_type text not null check (task_type in ('filter_change', 'sump_flush', 'other')),
  primary key (maintenance_log_id, task_type)
);
...
grant select, insert, update, delete on core.maintenance_log_tasks to authenticated;

create policy "maintenance_log_tasks_read_active_member" on core.maintenance_log_tasks
  for select to authenticated
  using (core.is_active_member());

create policy "maintenance_log_tasks_insert_contributor" on core.maintenance_log_tasks
  for insert to authenticated
  with check (core.is_contributor());

create policy "maintenance_log_tasks_admin_all" on core.maintenance_log_tasks
  for all to authenticated
  using (core.is_admin())
  with check (core.is_admin());
