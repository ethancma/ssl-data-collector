-- maintenance_logs — one ad hoc maintenance action (filter change, sump flush, or
-- other) for one system per row, following the chemical_additions pattern
-- (data_source + event/entered timestamps, recorded_by defaulted server-side,
-- contributor/admin RLS). Scoped down from the roadmap's full "Maintenance
-- scheduling" area (platform-architecture.md §4, §6) — no recurrence/due-date
-- maintenance_tasks table, no interval config, no due/overdue computation, just the
-- event log.
...
create table core.maintenance_logs (
  ...
  task_type text not null
    constraint maintenance_logs_task_type_check
      check (task_type in ('filter_change', 'sump_flush', 'other')),
...
comment on table core.maintenance_logs is
  'One performed maintenance action for one system per row.';
comment on column core.maintenance_logs.task_type is
